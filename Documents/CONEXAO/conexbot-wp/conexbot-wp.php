...
function conexbot_save_bot_id_ajax_handler() {
    ...
    $chat_type = isset($_POST['chat_type']) ? sanitize_text_field($_POST['chat_type']) : 'whatsapp';
    $whatsapp = isset($_POST['whatsapp']) ? sanitize_text_field($_POST['whatsapp']) : '';
    
    update_option('conexbot_bot_id', $bot_id);
    update_option('conexbot_chat_type', $chat_type);
    if (!empty($whatsapp)) update_option('conexbot_whatsapp_number', $whatsapp);
...
add_action('wp_footer', function() {
    ...
    $chat_type = get_option('conexbot_chat_type', 'whatsapp');
    ...
    if ($chat_type === 'whatsapp') {
        ...
        ?>
        <style>
            .conexbot-chat-widget { position: fixed; bottom: 30px; right: 30px; z-index: 99999; transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
            .conexbot-chat-button { width: 65px; height: 65px; background: #25d366; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 10px 25px rgba(37, 211, 102, 0.4); cursor: pointer; text-decoration: none; position: relative; }
            .conexbot-chat-button:hover { transform: scale(1.1) translateY(-5px); box-shadow: 0 15px 30px rgba(37, 211, 102, 0.5); }
            .conexbot-chat-button svg { width: 35px; height: 35px; fill: #fff; }
            .conexbot-chat-tooltip { position: absolute; right: 80px; top: 50%; transform: translateY(-50%); background: #fff; color: #1a1a1a; padding: 10px 18px; border-radius: 12px; font-family: inherit; font-size: 14px; font-weight: 600; white-space: nowrap; box-shadow: 0 5px 15px rgba(0,0,0,0.1); opacity: 0; visibility: hidden; transition: all 0.2s; }
            .conexbot-chat-widget:hover .conexbot-chat-tooltip { opacity: 1; visibility: visible; right: 90px; }
        </style>
        ...
        // Chat IA Nativo
        $embed_url = "https://app.conext.click/embed/chat/" . $bot_id;
        ?>
        <style>
            #conexbot-native-chat { position: fixed; bottom: 30px; right: 30px; z-index: 99999; }
        ...
        <div id="conexbot-native-chat">
            <div id="conexbot-iframe-container">
                <iframe src="<?php echo esc_url($embed_url); ?>" style="width: 100%; height: 100%; border: none;"></iframe>
            </div>
            <div class="conexbot-bubble" onclick="document.getElementById('conexbot-iframe-container').style.display = (document.getElementById('conexbot-iframe-container').style.display === 'block' ? 'none' : 'block')">
