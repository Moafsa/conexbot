        ...
        foreach ($product_ids as $product_id) {
            switch ($action) {
                ...
                case 'pause_on_ml':
                    ...
                    break;
                case 'activate_on_ml':
                    if (!is_wp_error(TS_ML_Product_Sync::instance()->activate_listing($product_id, $account_id))) {
                        $count_success++;
                    }
                    break;
...
if (isset($_GET['ml_status_action']) && isset($_GET['product_id']) && isset($_GET['account_id']) && check_admin_referer('ml_status_action_' . $_GET['product_id'])) {
    ...
    if ($status_action === 'close') {
        $result = $product_sync->close_listing($product_id, $account_id);
        $success_msg = __('Anúncio finalizado no Mercado Livre.', 'ts-ml-integration');
