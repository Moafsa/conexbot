...
with open('src/app/dashboard/marketing/page.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# Define the new CampaignModal component string
new_modal = '''function CampaignModal({ onClose, selectedClientId, onSuccess }: any) {
...
# Use regex to find and replace the CampaignModal function
pattern = re.compile(r'function CampaignModal\(\s*\{.*?\}\s*\)\s*\{.*?return\s*\(.*?\);\s*\}', re.DOTALL)
new_content = pattern.sub(new_modal, content)

with open('src/app/dashboard/marketing/page.tsx', 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Updated CampaignModal in page.tsx")
